    ...
    let fit = env.stream(source.clone())
        .replay(
            ...
            {   
                ...
                state.weights = state.weights.iter().zip(adam.iter()).map(|(wi,a)| wi - learn_rate*a - state.weight_decay as f64 * learn_rate * 0.002 * wi).collect();
