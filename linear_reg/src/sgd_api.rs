...
fn main() {
    ...
    let num_features = 5;
    let normalize = true;
    let path_to_data = "housing_numeric.csv".to_string();
    ...
    model = model.fit(path_to_data.clone(), num_iters, learn_rate, batch_size, normalize, l2_reg, config.clone());

    let r2 = model.clone().score(path_to_data, config);
