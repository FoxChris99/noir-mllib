...
use noir_ml::{
    adam_regressor::linear_adam,
    basic_stat::get_moments,
    ols_regressor::{ols_training, ols_training_array},
    sample::Sample,
    sgd_regressor::{linear_batch_gd, linear_sgd},
};
        ...
        match method.as_str() {
            "ADAM" | "adam" => {
                let state = linear_adam(
                    ...
                    n_iter_no_change,
                    normalize,
                    self.train_mean.clone(),
                    self.train_std.clone(),
                    config,
                    "None",
                    0.,
                );
...
impl LinearRegression {
    fn fit_ols(&mut self, path_to_data: &String, normalize: bool, config: &EnvironmentConfig) {
        ...
        let weights = ols_training_array(
            path_to_data,
            normalize,
            self.train_mean.clone(),
            self.train_std.clone(),
...
impl LinearRegression {
    fn score(&self, path_to_data: &String, config: &EnvironmentConfig) -> f64 {
        if self.fitted != true {
            panic!("Can't compute score before fitting the model!");
        }
        let mut avg_y = 0.;
        let source = CsvSource::<Sample>::new(path_to_data)
            .has_headers(true)
            .delimiter(b',');
        ...
        if self.normalization == false {
            ...
            let res = env
                .stream(source.clone())
                .group_by_avg(|_x| true, move |x| x.0[x.0.len() - 1])
                .drop_key()
                .collect_vec();
            env.execute();
        ...
        let score = env
            ...
            .map(move |mut x| {
                ...
                if normalization == true {
                    x.0 =
                        x.0.iter()
                            .zip(train_mean.iter().zip(train_std.iter()))
                            .map(|(xi, (m, s))| (xi - m) / s)
                            .collect();
                    mean_y = 0.;
                }
...
impl LinearRegression {
    fn predict(&self, path_to_data: &String, config: &EnvironmentConfig) -> Vec<f64> {
        if self.fitted != true {
            panic!("Can't compute predictions before fitting the model!");
        }
        let source = CsvSource::<Sample>::new(path_to_data)
            .has_headers(true)
            .delimiter(b',');
