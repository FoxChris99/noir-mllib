                        ...
                        .rich_filter_map({
                            ...
                            move |mut x|{
                                ...
                                if count < batch_size * (state.get().epoch+1) {
                                    ...
                                    Some(Sample(current_weights.iter().zip(sample_grad.iter()).map(|(wi,gi)| gi + if *wi>=0. {lambda} else {-lambda}).collect()))
                                    }
                                else {None}}})
...
fn main() {
    ...
    let mut model = LassoRegression::new();
