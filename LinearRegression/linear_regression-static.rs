...
lazy_static! {
    static ref STATIC_DATA: Mutex<(Array2<f64>, Array1<f64>)> = Mutex::new(data_from_csv("c").unwrap());
    static ref STATIC_FEATURES: Mutex<Array2<f64>> = Mutex::new(data_from_csv("c").unwrap().0);
}

// fn get_data(filename: &str) -> Option<(&'static Array2<f64>, &'static Array1<f64>)> {
//     let mut data = STATIC_DATA.lock().unwrap();
//     if data.is_none() {
//         let (features, target) = data_from_csv(filename).ok()?;
//         *data = Some((features, target));
//     }
//     let (features, target) = data.as_ref()?;
//     Some((features, target))
// }

// fn populate_static_data(filename: &str) {
//         let mut data = STATIC_DATA.lock().unwrap();
//         if data.is_none() {
//             let (features, target) = data_from_csv(filename).unwrap();
//             *data = Some((features, target));
//         }
//     }
